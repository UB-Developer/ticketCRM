...
export default function DashboardPage() {
    ...
    // 2. Data Definitions (Stats & Tables)
    const stats = [
        {
                    ...
                    <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                        <div>
                            ...
                            <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
                                Logged in as <span className="font-semibold text-cyan-500">{user.role}</span>
                            </p>
                        </div>
                    ...
                    <div className="mt-6 grid grid-cols-1 gap-6 xl:grid-cols-3">
                        ...
                        <div className="xl:col-span-2 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-white/10 dark:bg-white/[0.03]">
                            <div className="flex items-center justify-between">
                                ...
                                <button type="button" className="rounded-lg border border-slate-200 px-3 py-2 text-xs font-medium text-slate-600 dark:border-white/10 dark:text-slate-300">
                                    This Year
                                </button>
                            </div>
                            {/* Fake Chart */}
                            <div className="mt-8 flex h-[260px] items-end gap-3 sm:gap-5">
                                {[42, 58, 48, 72, 63, 82, 68, 90, 76, 96, 84, 100].map((height, index) => (
                                    <div key={index} className="flex flex-1 flex-col items-center gap-2">
                                        <div className="flex h-[210px] w-full items-end">
                                            <div className="w-full rounded-t-lg bg-gradient-to-t from-blue-600 to-cyan-400 transition-all hover:opacity-80" style={{ height: `${height}%` }} />
                                        </div>
                        ...
                        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-white/10 dark:bg-white/[0.03]">
                            ...
                            <div className="mt-6 space-y-3">
                                {[
                                    { title: "Create Booking", icon: Ticket },
                                    { title: "Add Passenger", icon: Users },
                                    { title: "Create Travel Group", icon: Plane },
                                    { title: "View Reports", icon: TrendingUp },
                                ].map((action) => {
                                    const Icon = action.icon;
                                    return (
                                        <button key={action.title} type="button" className="flex w-full items-center gap-3 rounded-xl border border-slate-200 p-3 text-left transition hover:border-cyan-400 hover:bg-cyan-50 dark:border-white/10 dark:hover:bg-cyan-500/5">
                                            <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-cyan-500/10 text-cyan-500">
                                                <Icon className="h-5 w-5" />
                                            </div>
                                            <span className="flex-1 text-sm font-medium text-slate-700 dark:text-slate-200">{action.title}</span>
                    ...
                    <div className="mt-6 rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-white/10 dark:bg-white/[0.03]">
                        <div className="flex flex-col gap-3 border-b border-slate-200 p-6 sm:flex-row sm:items-center sm:justify-between dark:border-white/10">
                            <div>
                                ...
                                <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Latest booking activity</p>
                            </div>
                            <button type="button" className="text-sm font-semibold text-cyan-500 hover:text-cyan-400">View All</button>
                        </div>
                        <div className="overflow-x-auto">
                            <table className="w-full min-w-[700px]">
                                <thead>
                                    <tr className="border-b border-slate-200 dark:border-white/10">
                                        <th className="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider text-slate-500">Booking</th>
                                        <th className="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider text-slate-500">Passenger</th>
                                        <th className="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider text-slate-500">Package</th>
                                ...
                                <tbody>
                                    {recentBookings.map((booking) => (
                                        <tr key={booking.id} className="border-b border-slate-100 transition hover:bg-slate-50 dark:border-white/5 dark:hover:bg-white/[0.02]">
                                            <td className="px-6 py-4"><span className="text-sm font-semibold text-cyan-500">{booking.id}</span></td>
                                            <td className="px-6 py-4"><span className="text-sm font-medium text-slate-800 dark:text-slate-200">{booking.passenger}</span></td>
                                            <td className="px-6 py-4"><span className="text-sm text-slate-500 dark:text-slate-400">{booking.package}</span></td>
                                            <td className="px-6 py-4"><span className="text-sm font-semibold text-slate-800 dark:text-slate-200">{booking.amount}</span></td>
                    ...
                    <div className="mt-6 grid grid-cols-1 gap-6 md:grid-cols-2">
                        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-white/10 dark:bg-white/[0.03]">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h3 className="font-semibold text-slate-900 dark:text-white">Pending Payments</h3>
                                    <p className="text-sm text-slate-500 dark:text-slate-400">Payments waiting confirmation</p>
                                </div>
                            </div>
                            <div className="mt-5"><span className="text-3xl font-bold text-slate-900 dark:text-white">₨ 1.24M</span></div>
                        </div>
                        ...
                        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-white/10 dark:bg-white/[0.03]">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h3 className="font-semibold text-slate-900 dark:text-white">Upcoming Departures</h3>
                                    <p className="text-sm text-slate-500 dark:text-slate-400">Groups departing soon</p>
                                </div>
                            </div>
                            <div className="mt-5">
                                <span className="text-3xl font-bold text-slate-900 dark:text-white">12</span>
                                <span className="ml-2 text-sm text-slate-500">groups</span>
                            </div>
                        </div>
                    </div>
        </div>
